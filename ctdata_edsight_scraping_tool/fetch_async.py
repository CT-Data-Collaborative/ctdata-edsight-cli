...
def get_xpaths(filters, variables):
    return [f['xpath_id'] for f in filters if f['name'] in variables]


def build_url_list(params, xpaths, url, output_dir, dataset_name):
...
def setup_download_targets(dataset, output_dir, variable):
    ...
    xpaths = get_xpaths(ds_filters, variable)
    ...
    return build_url_list(params, xpaths, new_url, output_dir, dataset)
